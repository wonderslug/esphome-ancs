...
static const uint8_t k_ancs_uuid_le[16] = {
...
#define MK_UUID128(b0,b1,b2,b3,b4,b5,b6,b7,b8,b9,ba,bb,bc,bd,be,bf) \
  { { BLE_UUID_TYPE_128 }, { b0,b1,b2,b3,b4,b5,b6,b7,b8,b9,ba,bb,bc,bd,be,bf } }

// Service:           7905F431-B5CE-4E99-A40F-4B1E122D00D0
static ble_uuid128_t s_ancs_svc_uuid = MK_UUID128(
    0xD0, 0x00, 0x2D, 0x12, 0x1E, 0x4B, 0x0F, 0xA4,
    0x99, 0x4E, 0xCE, 0xB5, 0x31, 0xF4, 0x05, 0x79);

// Notification Source: 9FBF120D-6301-42D9-8C58-25E699A21DBD
static ble_uuid128_t s_notif_src_uuid = MK_UUID128(
    0xBD, 0x1D, 0xA2, 0x99, 0xE6, 0x25, 0x58, 0x8C,
    0xD9, 0x42, 0x01, 0x63, 0x0D, 0x12, 0xBF, 0x9F);

// Control Point:       69D1D8F3-45E1-49A8-9821-9BBDFDAAD9D9
static ble_uuid128_t s_ctrl_point_uuid = MK_UUID128(
    0xD9, 0xD9, 0xAA, 0xFD, 0xBD, 0x9B, 0x21, 0x98,
    0xA8, 0x49, 0xE1, 0x45, 0xF3, 0xD8, 0xD1, 0x69);

// Data Source:         22EAC6E9-24D6-4BB5-BE44-B36ACE7C7BFB
static ble_uuid128_t s_data_src_uuid = MK_UUID128(
...
static void push_event(const BleEvent &ev) {
  ...
  if (xQueueSend(s_queue, &heap_ev, 0) != pdTRUE) {
    ...
    if (xQueueReceive(s_queue, &old, 0) == pdTRUE) delete old;
    if (xQueueSend(s_queue, &heap_ev, 0) != pdTRUE) {
      delete heap_ev;
...
// ---------------------------------------------------------------------------
#define MK_UUID16(val) ble_uuid16_t{{ BLE_UUID_TYPE_16 }, (val)}
...
// ---------------------------------------------------------------------------
static int read_device_name_cb(uint16_t conn_handle,
...
static void write_cccd_enable(uint16_t conn_handle, uint16_t val_handle) {
  ...
  ble_gattc_write_flat(conn_handle, val_handle + 1,
  ...
  switch (event->type) {
    ...
    case BLE_GAP_EVENT_ENC_CHANGE: {
      ...
      if (enc_handle != s_conn_handle) {
        ESP_LOGW(TAG, "stale enc_change for handle=%u (active=%u) — ignoring",
                 enc_handle, s_conn_handle);
        return 0;
      }
      ...
      if (event->enc_change.status == 0) {
        ...
        ESP_LOGW(TAG, "enc_change failed status=%d — terminating to recover",
    ...
    case BLE_GAP_EVENT_NOTIFY_RX: {
      ...
      uint16_t copy_len = (pkt_len < protocol::ANCS_ATTR_BUF_SIZE)
                              ? pkt_len
                              : (uint16_t) protocol::ANCS_ATTR_BUF_SIZE;
...
static int on_disc_chr(uint16_t conn_handle,
  ...
  if (error->status == BLE_HS_EDONE) {
    ...
    ESP_LOGI(TAG, "ANCS chars done: ns=%u cp=%u ds=%u",
             s_ns_handle, s_cp_handle, s_ds_handle);
    if (s_ns_handle) write_cccd_enable(conn_handle, s_ns_handle);
    if (s_ds_handle) write_cccd_enable(conn_handle, s_ds_handle);
    ...
    s_peer_name_buf[0] = '\0';
    int rc = ble_gattc_read_by_uuid(conn_handle, 1, 0xFFFF,
                                     u16p(&s_uuid_device_name),
                                     read_device_name_cb, nullptr);
    if (rc != 0) {
...
static void handle_notification_source(const uint8_t *buf, uint16_t len) {
  ...
  switch (ns.event_id) {
    case protocol::EventId::ADDED: {
      ...
      if (s_cfg.auto_fetch && s_cp_handle != 0 && !s_cfg.fetch_attributes.empty()
          && !s_fetch_pending) {
        ...
        size_t cmd_len = protocol::build_get_notification_attributes(
            ns.uid, reqs, n_reqs, cmd, sizeof(cmd));

        if (cmd_len > 0) {
          int rc = ble_gattc_write_flat(s_conn_handle, s_cp_handle,
                                        cmd, cmd_len, NULL, NULL);
          if (rc != 0) {
...
void AncsBle::init(const BleConfig &cfg) {
  if (s_queue == nullptr) s_queue = xQueueCreate(16, sizeof(BleEvent *));
  s_cfg = cfg;
...
void AncsBle::loop() {
  ...
  if (!s_adv_override_pending) return;
  if (!ble_gap_adv_active()) return;  // not started yet — retry next loop
  ...
  if (adv_name_len > 8) adv_name_len = 8;

  uint8_t adv[31];
  int adv_len = 0;
  adv[adv_len++] = 0x02;  adv[adv_len++] = 0x01;  adv[adv_len++] = 0x06;
  adv[adv_len++] = 0x11;  adv[adv_len++] = 0x15;
  memcpy(&adv[adv_len], k_ancs_uuid_le, 16);  adv_len += 16;
  adv[adv_len++] = (uint8_t)(adv_name_len + 1);
  adv[adv_len++] = shortened ? 0x08 : 0x09;
  memcpy(&adv[adv_len], dev_name, adv_name_len);  adv_len += (int) adv_name_len;

  // --- Scan-response packet (max 31 bytes) ---
  const char *model = s_cfg.model.c_str();
  size_t model_len = strlen(model);
  if (model_len > 20) model_len = 20;  // clamp to prevent size_t underflow below
  ...
  if (scan_name_len > scan_name_max) scan_name_len = scan_name_max;

  uint8_t scan[31];
  int scan_len = 0;
  scan[scan_len++] = (uint8_t)(scan_name_len + 1);
  scan[scan_len++] = 0x09;
  memcpy(&scan[scan_len], dev_name, scan_name_len);  scan_len += (int) scan_name_len;
  scan[scan_len++] = (uint8_t)(1 + 2 + model_len);  // length byte uses clamped model_len
  scan[scan_len++] = 0xFF;
  scan[scan_len++] = 0xFF;
  scan[scan_len++] = 0xFF;
  memcpy(&scan[scan_len], model, model_len);  scan_len += (int) model_len;  // clamped model_len

  int rc1 = ble_gap_adv_set_data(adv, adv_len);
  int rc2 = ble_gap_adv_rsp_set_data(scan, scan_len);

  ESP_LOGD(TAG, "adv override: adv(%d B) rc=%d  scan(%d B) rc=%d",
           adv_len, rc1, scan_len, rc2);
...
bool AncsBle::pop_event(BleEvent &out) {
  BleEvent *ev = nullptr;
  if (s_queue == nullptr || xQueueReceive(s_queue, &ev, 0) != pdTRUE) return false;
  out = *ev;
  delete ev;
  return true;
}
...
void AncsBle::request_attributes(uint32_t uid) {
  if (s_cp_handle == 0 || s_cfg.fetch_attributes.empty()) return;
  ...
  uint8_t cmd[protocol::ANCS_ATTR_BUF_SIZE];
  size_t cmd_len = protocol::build_get_notification_attributes(
      uid, reqs, n_reqs, cmd, sizeof(cmd));
