...
const char *category_to_string(Category c) {
  switch (c) {
    ...
    case Category::INCOMING_CALL: return "incoming_call";
    case Category::MISSED_CALL: return "missed_call";
    case Category::VOICEMAIL: return "voicemail";
    ...
    case Category::HEALTH_FITNESS: return "health_fitness";
    case Category::BUSINESS_FINANCE: return "business_finance";
    case Category::LOCATION: return "location";
...
const char *event_id_to_string(EventId e) {
  switch (e) {
    case EventId::ADDED: return "added";
    case EventId::MODIFIED: return "modified";
    case EventId::REMOVED: return "removed";
  }
  return "unknown";
}
...
bool parse_notification_source(const uint8_t *data, uint16_t len, NotificationSource &out) {
  ...
  out.category = (data[2] <= static_cast<uint8_t>(Category::ENTERTAINMENT))
                     ? static_cast<Category>(data[2])
                     : Category::OTHER;
...
DataSourceAssembler::Status DataSourceAssembler::parse_buffer_() {
  ...
  if (!header_checked_) {
    if (buf_.size() < 5) return Status::NEED_MORE;
  ...
  while (true) {
    if (buf_.size() - pos < 3) break;  // need id + len
    ...
    if (buf_.size() - pos - 3 < alen) break;  // value incomplete — wait for more
    pos += 3;
    std::string val(reinterpret_cast<const char *>(buf_.data() + pos), alen);
    bool found = false;
    for (auto &v : values_)
      if (v.first == aid) { v.second = val; found = true; break; }
    if (!found) values_.emplace_back(aid, val);
    pos += alen;
  }
  ...
  for (AttributeId want : expected_) {
    bool have = false;
    for (auto &v : values_)
      if (v.first == want) { have = true; break; }
    if (!have) return Status::NEED_MORE;
  }
